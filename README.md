...
2. Allow the scripts to recompile, and 2 new auto-generated scripts will have been added to:
...
* `struct` `[BoxGroup]` does not work as expected, causing the struct to remain a Foldout Group once inside the box group
* `[Button]` with a dynamic label does not appear to work as expected
# Change Log
